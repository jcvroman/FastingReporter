...
// NOTE: BDD: Gherkin language: Gherkin is a business readable language which helps you to describe business behavior without going into details of implementation.
//            It is a domain specific language for defining tests in Cucumber format for specifications. It uses plain language to describe use cases and allows users
//            to remove logic details from behavior tests.
// NOTE: I'm using the Gherkin language for Acceptance tests (i.e. BDD) and all other tests (e.g. Unit tests), but not Cucumber.
